...
class Enseignant(models.Model):
    id_enseignant = models.OneToOneField(
        Utilisateur,
...
class Emplacement(models.Model):
    ...
    id_enseignant_principal = models.ForeignKey(
        Enseignant,
        on_delete=models.CASCADE,
        blank=False,
        null=True,
        db_column="id_enseignant_principal",
        to_field="id_enseignant",
        related_name="enseignant_principal",
    )
    id_enseignant_secondaire = models.ForeignKey(
        Enseignant,
        on_delete=models.CASCADE,
        blank=False,
        null=True,
        db_column="id_enseignant_secondaire",
        to_field="id_enseignant",
        related_name="enseignant_secondaire",
    )
    ...
    try:
        if value < 0.0 or value > 20.0:
            raise ValidationError(
                _(f"(value) must be in the range [0.0, 20.0]"),
            )
        return round(float(value), 2)
    except:
        raise ValidationError(
            _(f"(value) is not an integer or a float number"),
        )


class Candidat(models.Model):
    id_candidat = models.OneToOneField(
        Utilisateur,
        on_delete=models.CASCADE,
        blank=False,
        null=False,
        db_column="id_candidat",
        to_field="id",
        primary_key=True,
    )
    universite = models.CharField(max_length=200, blank=False, null=False)
    code_anonyme = models.CharField(
        default=None, db_index=True, max_length=10, unique=True, blank=False, null=True
    )
    faculte = models.CharField(max_length=100, blank=False, null=False)
    ...
    note_sujet1 = models.FloatField(
        default=0.00, validators=[validate_decimals], null=False, blank=False
    )
    note_sujet2 = models.FloatField(
        default=0.00, validators=[validate_decimals], null=False, blank=False
    )
...
class Notification_Candidats(models.Model):
    ...
    id_candidat = models.ForeignKey(
        Candidat,
        on_delete=models.CASCADE,
        blank=False,
        null=False,
        db_column="id_candidat",
        to_field="id_candidat",
    )
...
class Correction(models.Model):
    ...
    code_anonyme_candidat = models.ForeignKey(
        Candidat,
        on_delete=models.CASCADE,
        blank=False,
        null=True,
        db_column="code_anonyme_candidat",
        to_field="code_anonyme",
    )
    ...
    numero_de_correction = models.IntegerField(
        choices=NB_CORRECTION, null=False, blank=False
    )
    ETAT = [
