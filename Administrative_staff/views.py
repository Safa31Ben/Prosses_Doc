...
def getRessourcesHumains(request):
    if request.method == "GET":
        ...
        enseignants = Utilisateur.objects.exclude(Q(type="candidat") | Q(type="admin")).values(
            "enseignant__grade",
            "enseignant__faculte",
...
def getEnseignantsEtSujet(request, id):
    if request.method == "GET":
        concours = (
            Concours.objects.all().order_by("-annee_concours").first().id_concours
        )
        sujets = Sujet.objects.filter(id_concours=concours).values(
            "id_sujet", "description", "type"
        )
        fac = Enseignant.objects.get(id_enseignant=id).faculte
        enseignants = (
            Enseignant.objects.extra(
                select={
            ...
            .filter(faculte=fac)
            .values(
                "id_enseignant", "nom", "prenom", "grade", "depertement", "specialite"
            )
        )
        nb_total_copie = Candidat.objects.filter(
            id_concours=concours, faculte=fac, code_anonyme__isnull=False
        ).count()
        if sujets:
            return Response(
                {
            ...
            return Response(
                {"sujets": "Les sujets de cette concours pas encore ajouté"}
            )
...
def setEnseignantsEtSujet(request, id):
    if request.method == "PUT":
        ...
        for key, value in enseignants.items():
            ...
            candidats_have_corrector = (
                Correction.objects.all()
                .filter(numero_de_correction=value.get("nb_corr"), id_enseignant=key)
                .values("code_anonyme_candidat")
            )
            candidats_havent_corrector = list(
                candidats.exclude(code_anonyme__in=candidats_have_corrector).values(
                    "code_anonyme"
                )
            )[0 : value.get("nb_copie")]
            for candidat in candidats_havent_corrector:
                id_candidat = Candidat.objects.get(
                    code_anonyme=candidat.get("code_anonyme")
                )
                Correction(
                    id_enseignant=enseignant,
                    code_anonyme_candidat=id_candidat,
                    numero_de_correction=value.get("nb_corr"),
                ).save()
...
def generCodesAnonyme(request):
    if request.method == "GET":
        ...
        candidats = (
            Candidat.objects.all().filter(id_concours=concours).values("id_candidat")
        )
        ...
        candidats = (
            ...
            .filter(id_concours=concours, code_anonyme__isnull=False)
            .values(
                "id_candidat",
                "universite",
                "code_anonyme",
    ...
    if request.method == "GET":
        ...
        for candidat in candidats:
            ...
            if abs(note1.get("note") - note2.get("note")) < 3:
                ...
                n2.etat = "valide"
                n2.save()
                c = Candidat.objects.get(
                    code_anonyme=candidat.get("code_anonyme_candidat")
                )
                c.note_sujet1 = sum([n1.note, n2.note]) / 2
                c.save()
            else:
                sujet = Enseignant.objects.get(
                    id_enseignant=note1.get("id_enseignant_id")
                ).id_sujet.id_sujet
                ...
                if sujet.get("id_sujet") not in sujets:
                    enseignants = (
                        ...
                        .filter(faculte=fac, id_sujet=None)
                        .values(
                            "id_enseignant",
                            "nom",
                            "prenom",
                    ...
                    (sujets.get(sujet.get("id_sujet")))["candidats"].append(
                        candidat.get("code_anonyme_candidat")
                    )
    ...
    if request.method == "GET":
        ...
        for candidat in candidats:
            ...
            if note1 != 0 and note2 != 0:
                ...
                id_enseignant = Correction.objects.filter(
                    code_anonyme_candidat=candidat.get("code_anonyme"),
                    numero_de_correction=3,
                    ...
                    for s in sujetQ:
                        ...
                        for enseignant in enseignants:
                            note = (
                                Correction.objects.filter(
                                    code_anonyme_candidat=candidat.get("code_anonyme"),
                                    id_enseignant=enseignant.get("id_enseignant_id"),
                                )
                                .values()
                                .first()
                            )
                        ...
                        if note1 == 0 and note2 != 0:
                            c = Candidat.objects.get(
                                id_candidat=candidat.get("id_candidat_id")
                            )
        ...
        return Response(
            {"Etat": "La validation des notes et le calcul de moyennes est effectuée"}
        )
...
def getDisponibleEmplacementEnseignants(request, id):
    if request.method == "GET":
        ...
        emplacements = Emplacement.objects.filter(
            universite=enseignant.universite, faculte=enseignant.faculte
        ).values("id_emplacement", "salle", "capacite")
        enseignants = (
            Enseignant.objects.extra(
                select={
                    "nom": "select nom from utilisateur where utilisateur.id=id_enseignant",
            ...
            .filter(universite=enseignant.universite, faculte=enseignant.faculte)
            .values("id_enseignant", "nom", "prenom", "grade")
        )
        nb_candidats = Candidat.objects.filter(
            id_concours=concours, faculte=enseignant.faculte
        ).count()
        emplacement = []
        co = 0
        for empl in emplacements:
            if co < nb_candidats:
                ...
                break
        return Response({"emplacements": emplacement, "enseignants": enseignants})
...
def setEmplacementEnseignantsCandidats(request, id):
    if request.method == "PUT":
        ...
        for key, value in emplacements.items():
            ...
            emplacement.id_enseignant_principal = Enseignant.objects.get(
                id_enseignant=value.get("id_enseignant_principal")
            )
            emplacement.id_enseignant_secondaire = Enseignant.objects.get(
                id_enseignant=value.get("id_enseignant_secondaire")
            )
            emplacement.save()

            candidats_havent_emplacement = list(
                ...
                .values("id_candidat_id")
            )[0 : emplacement.capacite]
        ...
        return Response(
            {
                "emplacements": "L'affectation des enseignants pour la surveillance et les emplacements des candidats été effectée"
            }
        )
...
def affecterSujetThese(request, id):
    if request.method == "GET":
        ...
        for candidat in candidats:
            choises = (
                Choix.objects.filter(id_candidat=candidat.get("id_candidat_id"))
                .order_by("order")
                .values()
            )
            for choix in choises:
                given = Choix.objects.filter(
                    etat=True, id_these=choix.get("id_these_id")
                )
                if not given:
        ...
        for these in theses:
            can = (
                Candidat.objects.filter(
                    id_concours=concours,
                    faculte=fac,
                    id_candidat=these.get("id_candidat_id"),
                )
                .extra(
                    select={
                        "nom": "select nom from utilisateur where utilisateur.id=id_candidat",
...
def consulterStats(request, id):
    if request.method == "GET":
        ...
        nb_candidats = (
            ...
            .values("concours")
            .annotate(nb_candidats=Count("id_concours"))
            .order_by("concours")
        )
        ...
        presence_percentage = (
            ...
            .values("concours", "etat_presence")
            .annotate(nb_presence=Count("id_concours_id"))
            .order_by("concours")
        )
